...
export default function ArrivedAtLocalHub() {
  const navigate = useNavigate();
  const { data, reFetch } = useFetch("arrived_at_local_hub");
  ...
  const delivered_single_order = async (id) => {
    SetLoading(true)
    const res = await PostApi("delivered_single_order", {
      bookingId: id
    });
  ...
  const order_detail = (id) => {
    navigate('/arrived_at_local_hub_order_details', { state: { bookingId: id } })
  }
  const all = [];
          ...
          action: (
            <>
             <div className="flex gap-x-1">
                ...
                <button onClick={() => delivered_single_order(order.id)} className="py-1 w-[100px] bg-themeBlue text-white rounded border border-themeBlue hover:text-themeBlue hover:bg-white">Delivered</button>
              </div>
            </>
