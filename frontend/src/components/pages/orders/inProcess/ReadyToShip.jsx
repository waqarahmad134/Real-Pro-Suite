...
export default function ReadyToShip() {
  ...
  const { data,reFetch } = useFetch("ready_to_ship");
  const [loading,SetLoading] = useState(false);
  const order_detail = (id) => {
    navigate('/arrived_orders/details', { state: { bookingId: id } })
  }
  const intransit_single_order =async (id) => {
    SetLoading(true)
    const res = await PostApi("intransit_single_order", {
      bookingId: id
    });
          ...
          action: (
            <>
              <div className="flex gap-x-1">
                  ...
                  <button onClick={() => intransit_single_order(order.id)} className="py-1 w-[80px] bg-themeBlue text-white rounded border border-themeBlue hover:text-themeBlue hover:bg-white">InTransit</button>
