...
export default function DelayedOrders() {
  ...
  const { data } = useFetch("delayed_orders");

  const [query, setQuery] = useState("");
  const [loading, SetLoading] = useState(false);
  const order_detail = (id) => {
    navigate("/delayed_order_details", { state: { bookingId: id } });
  };
  const columns = [
    {
      name: "Sr #",
      ...
      <section className="relative xl:w-[calc(100%-280px)] w-[calc(100%-240px)] top-[148px] py-5 px-10 float-right space-y-8">
        <div className="flex items-center justify-between">
          <h1 className="flex items-center gap-x-3">
            ...
            <span className="font-bold text-2xl">Delayed</span>
          </h1>
